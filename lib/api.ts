...
export function getFileStructure(dir: string = mdDir): any {
  const result = {
    files: [],
    directories: [],
  };
  ...
  files.forEach((file) => {
    ...
    if (stat.isDirectory()) {
      ...
      const post = getPostBySlug(path.relative(mdDir, filePath), ["title"]);
      result.files.push({
        slug: path.relative(mdDir, filePath).replace(/\.md$/, ""),
        title: post.title,
      });
    }
  });
